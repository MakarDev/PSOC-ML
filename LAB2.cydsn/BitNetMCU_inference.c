    ...
    for (i = 0; i < n_output; i++) {
        ...
        // Multiplier-less inference for RV32EC
            ...
            for (k = 0; k < n_input; k+=8) {
                ...
                for (j = 0; j < 8; j++) {
                    in=*activations_idx++;
                    weight = (weightChunk) >> (32-4); // extend sign, cut off lower bits
                    sum += in*weight;
                    weightChunk <<= 4;
                }
            }
        } else if (bits_per_weight == 8 + 8 ) {   // 8 bit twos-complement
            for (k = 0; k < n_input; k+=4) {
                weightChunk = *weightidx++;
                for (j = 0; j < 4; j++) {
                    in=*activations_idx++;
                    weight = (weightChunk) >> (32-8); // extend sign, cut off lower bits
                    sum += in*weight;
                    weightChunk <<= 8;
                }
            }
