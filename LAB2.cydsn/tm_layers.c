...
{   TM_PERF_INIT(t_sbuf);TM_PERF_INIT(t_dotp);TM_PERF_INIT(t_post);
    ...
    if(maxk==1){ TM_PERF_START(t_pwconv);   //pointwise conv
        ...
        int c = 0;
        mtype_t* sptr = in->data;  // Add sptr declaration
        wtype_t* kptr = w;         // Add kptr declaration
        for(; c<out->c-BATCH_SIZE+1; ){
            int bat;
            for(bat = 0; bat < BATCH_SIZE; bat+=2)
...
// Custom exponential function implementation using Taylor series approximation
static float custom_exp(float x) {
