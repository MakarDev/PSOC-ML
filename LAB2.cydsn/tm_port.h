...
#define TM_DBGT_INIT()     struct { uint32_t start; uint32_t finish; float time; } _tm_timer = {0,0,0};
#define TM_DBGT_START()    _tm_timer.start = TM_GET_US();
#define TM_DBGT(x)         {_tm_timer.finish = TM_GET_US();\
                            _tm_timer.time = (float)(_tm_timer.finish-_tm_timer.start)/1000.0;\
                            TM_PRINTF("===%s use %.3f ms\n", (x), _tm_timer.time);\
                            _tm_timer.start = TM_GET_US();}
